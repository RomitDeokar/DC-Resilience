...
export function configError(c: Config): string {
  ...
  if ((sim.dependent_failures||sim.diagnostics)&&sim.common_cause_target==='application'&&!c.it.enabled) return 'Shared application failures require the IT service model to be enabled.'
...
export function workEstimate(c: Config, compare = false) {
  ...
  const activeRates=5+(c.it.enabled?5:0)-(c.simulation.operating_mode==='utility'?1:0)
  const diagnosticPasses=c.simulation.diagnostics?2+2*activeRates+(c.simulation.operating_mode==='islanded'?1:0):0
  const totalTrials=c.simulation.num_trials*(compare?3:1)+Math.min(c.simulation.num_trials,1000)*diagnosticPasses
  return {components, diagnosticPasses, totalTrials, unitTrials: weight*c.simulation.num_trials, maxTrials: Math.min(20000,Math.floor(45000000/weight))}
}
