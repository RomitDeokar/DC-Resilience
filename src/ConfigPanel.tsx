...
import { DEFAULT_CONFIG, TIERS, number, configError, type Config, type Redundancy } from './types'
...
export function ConfigPanel({config, setConfig, busy, onRun, compare = false}: {config: Config; setConfig: (c: Config) => void; busy: boolean; onRun: () => void; compare?: boolean}) {
  ...
  function preset(value: string) {
    ...
    if (value === 'stress') { next.simulation.num_trials = 5000; next.simulation.stress_multiplier = 20 }
    if (value === 'single') next.simulation.failure_mode = 'single'
    if (value === 'paths') next.power.redundancy = next.cooling.redundancy = '2N'
    setConfig(next)
  }
  ...
  return <aside className="config-panel panel">
    ...
    <form onSubmit={e => {e.preventDefault(); onRun()}}>
      <fieldset disabled={busy}>
        <div className="preset-section"><label htmlFor="preset">DEMO STARTING POINT</label><select id="preset" value="" onChange={e=>preset(e.target.value)}><option value="">Load a preset…</option><option value="quick">Quick check · 1,000 trials</option><option value="baseline">Research baseline · 10,000 trials</option><option value="stress">Overlap stress demo · 20× hazards</option><option value="single">Single-failure protection</option><option value="paths">Independent paths · 2N</option></select><small>Presets reset all fields. Results update only when you run.</small></div>
