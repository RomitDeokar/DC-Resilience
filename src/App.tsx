...
const TITLES: Record<Page,[string,string]> = {overview:['Operations overview','One clear view of your facility. Every decision, better informed.'],designer:['Infrastructure designer','Shape your facility. Validate every layer.'],resilience:['Resilience assessment','Understand the margins. Find the single points of failure.'],calculator:['Capacity calculators','Engineering fundamentals, without the guesswork.'],faults:['Facility fault explorer','Test your design against the unexpected.'],dcim:['DCIM monitor','A live view of your synthetic facility telemetry.'],simulation:['Simulation workspace','Turn redundancy decisions into measurable reliability.'],lab:['Live failure lab','Explore what happens when your infrastructure fails.'],comparison:['Architecture comparison','Compare N, N+1 and 2N under identical conditions.'],history:['Run history','Your experiments, saved locally and ready to revisit.'],references:['Reference library','Traceable failure data. Clearly stated assumptions.'],methodology:['Methodology','An open, reproducible approach to reliability engineering.']}
...
export default function App() {
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content" onClick={e=>{e.preventDefault();document.getElementById('main-content')?.focus()}}>Skip to content</a>
    ...
    <aside className={`sidebar ${mobileOpen?'mobile-open':''}`}>
      <a className="brand" href="#overview" onClick={()=>nav('overview')}><span className="brand-mark"><Activity size={23} strokeWidth={2.5}/></span><div><strong>DC-Resilience<span className="brand-dot">.</span></strong><small>RELIABILITY, BY DESIGN</small></div></a>
      <div className="workspace-switch"><div className="workspace-avatar"><Server size={17}/></div><div><strong>Infrastructure intelligence</strong><span>SRM IST · Academic project</span></div><span className="workspace-badge">01</span></div>
