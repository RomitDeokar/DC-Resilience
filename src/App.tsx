...
import { availability, DEFAULT_CONFIG, number, configError, type Config, type FacilityState, type Page, type Rate, type Run } from './types'
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    loadRuns().then(runs=>{setHistory(runs.slice(0,12));const s=runs.find(r=>r.kind==='simulation');const c=runs.find(r=>r.kind==='comparison');if(s){setRun(s);setConfig(s.config)}if(c)setComparison(c)}).catch(()=>setToast('Browser history is unavailable. You can still run experiments and export results.'))
  },[])
  ...
  async function inject(ids: string[], label: string, hours=duration) {
    if(labLock.current||topologyBusy||!!configError(config))return
  ...
  function openHistory(r: Run) {if(busy||labBusy){setToast('Wait for the current operation before restoring a run.');return}setConfig(structuredClone(r.config));if(r.kind==='comparison')setComparison(r);else setRun(r);nav(r.kind==='comparison'?'comparison':'simulation');setToast('Saved experiment restored. Run again to reproduce it.')}
