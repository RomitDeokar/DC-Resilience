...
export default function App() {
  ...
  useEffect(()=>{document.title=`${TITLES[page][0]} · DC-Resilience`},[page])
  useEffect(()=>{const cb=()=>{setPage(getPage());setMobileOpen(false)};window.addEventListener('hashchange',cb);return()=>window.removeEventListener('hashchange',cb)},[])
  ...
  const newExperiment=()=>{if(busy||labBusy)return;setConfig(structuredClone(DEFAULT_CONFIG));setRun(null);setComparison(null);setError('');setConfigOpen(true);nav('simulation');setToast('New experiment ready. Configure your facility, then run the simulation.')}
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content" onClick={e=>{e.preventDefault();document.getElementById('main-content')?.focus()}}>Skip to content</a>
    ...
    <aside className={`sidebar ${mobileOpen?'mobile-open':''}`}>
      <a className="brand" href="#overview" onClick={()=>nav('overview')}><span className="brand-mark"><Activity size={23} strokeWidth={2.5}/></span><div><strong>DC-Resilience<span className="brand-dot">.</span></strong><small>DATA CENTRE ENGINEERING</small></div></a>
    ...
    <main id="main-content" tabIndex={-1} className={`main-content page-${page}`}><div className="page-heading"><div><div className="heading-eyebrow"><i/> DC / ENGINEERING CONSOLE</div><h1>{TITLES[page][0]}</h1><p>{TITLES[page][1]}</p></div><div className="heading-actions">{['simulation','comparison'].includes(page)&&<div className="export-container"><button className="button" disabled={!activeRun||busy} onClick={()=>setShowExport(!showExport)} aria-expanded={showExport}><Download size={15}/>Export report<ChevronDown size={13}/></button>{showExport&&<><div className="menu-catcher" onClick={()=>setShowExport(false)}/><div className="export-menu"><button onClick={()=>exportActive('json')}><FileJson size={16}/><div>Complete experiment<small>JSON · parameters, sources & all trials</small></div></button><button onClick={()=>exportActive('csv')}><FileSpreadsheet size={16}/><div>Results summary<small>CSV · comparison-ready results</small></div></button><button onClick={()=>exportActive('trials')}><Database size={16}/><div>Individual trial data<small>CSV · all simulated trials</small></div></button><button onClick={()=>activeRun&&report(activeRun)}><Printer size={16}/><div>Printable design report<small>Print or save as PDF</small></div></button></div></>}</div>}<button className="button dark" disabled={busy||labBusy} onClick={newExperiment}><Plus size={16}/>New experiment</button></div></div>
      {['simulation','comparison','lab'].includes(page)&&<div className="context-bar"><div><span><FlaskConical size={14}/>Monte Carlo simulation</span><i/><span>{config.simulation.dependent_failures?`${config.simulation.common_cause_target==='application'?'Shared deployment':'Common-cause'} + independent model`:'Independent failure model'}</span><i/><button onClick={()=>nav('references')}>Mixed-source assumptions <ArrowUpRight size={12}/></button></div><span className="context-right"><ShieldCheck size={13}/>Open source. Reproducible.</span></div>}
      ...
      {['simulation','comparison','lab'].includes(page)&&<div className={`workspace-grid ${page==='lab'?'lab-grid':''}`}>
        <div className="experiment-toolbar">
          <div className="experiment-summary"><span>IT DEMAND<strong>{number(config.it_load_kw)} <small>kW</small></strong></span><span>POWER / COOLING<strong>{config.power.redundancy} / {config.cooling.redundancy}</strong></span><span>EXPERIMENT<strong>{number(config.simulation.num_trials)} <small>trials · {config.simulation.simulated_years_per_trial} yr</small></strong></span></div>
          <div className="experiment-actions"><button className="button" aria-expanded={configOpen} aria-controls="experiment-config" onClick={()=>setConfigOpen(v=>!v)}><SlidersHorizontal size={15}/>{configOpen?'Close configuration':'Configure experiment'}<ChevronDown size={13} className={configOpen?'rotate':''}/></button>{page!=='lab'&&<button className="button primary" disabled={busy||labBusy||!!configError(config)} onClick={()=>void execute(page==='comparison')}><Play size={14}/>{busy?'Running…':page==='comparison'?'Run comparison':'Run experiment'}</button>}</div>
        </div>
        <div id="experiment-config" hidden={!configOpen}><ConfigPanel config={config} setConfig={setConfig} busy={busy||labBusy} compare={page==='comparison'} onRun={()=>void execute(page==='comparison')}/></div>
        <div className="workspace-main">{topologyError&&<div className="error-banner" role="alert"><Info size={18}/><p>{topologyError}</p></div>}
