...
import { availability, DEFAULT_CONFIG, number, type Config, type FacilityState, type Page, type Rate, type Run } from './types'
...
export default function App() {
  ...
  async function execute(compare=false, cfg=config, changePage=true) {
    if(busy) return
    setBusy(true);setElapsed(0);setError('');setShowExport(false)
    if(changePage) nav(compare?'comparison':'simulation')
    try {
      const data=await api<Run>(compare?'compare':'simulate',cfg)
      if(compare)setComparison(data);else setRun(data)
      setHistory(prev=>[data,...prev.filter(r=>r.run_id!==data.run_id)].slice(0,12))
      try {await saveRun(data);const all=await loadRuns();await Promise.all(all.slice(12).map(r=>deleteRun(r.run_id)))} catch {setToast('Run complete. Browser storage is unavailable; download JSON to keep your results.')}
      setToast(`${compare?'Comparison':'Simulation'} complete · ${number(data.results.reduce((a,r)=>a+r.trials_run,0))} trials in ${data.duration_seconds.toFixed(2)}s`)
      setHealth(true)
  ...
  useEffect(()=>{
    ...
    loadRuns().then(runs=>{setHistory(runs.slice(0,12));const s=runs.find(r=>r.kind==='simulation');const c=runs.find(r=>r.kind==='comparison');if(s){setRun(s);setConfig(s.config)}else{void execute(false,structuredClone(DEFAULT_CONFIG),false)}if(c)setComparison(c)}).catch(()=>{void execute(false,structuredClone(DEFAULT_CONFIG),false)})
  },[])
  ...
  useEffect(()=>{
    const controller=new AbortController();setFailed([]);setLogs([]);setTopologyBusy(true)
    const t=setTimeout(()=>{api<FacilityState>('topology',config,controller.signal).then(d=>{setState(d);setTopologyBusy(false)}).catch(e=>{if(e.name!=='AbortError'){setState(null);setTopologyBusy(false)}})},150)
  ...
  async function inject(ids: string[], label: string) {
    if(labBusy||topologyBusy)return
    setLabBusy(true);setError('')
    try {const s=await api<FacilityState>('inject-failure',{config,failed_components:ids,duration_hours:4});setState(s);setFailed(ids);setLogs(l=>[{time:new Date().toLocaleTimeString('en-GB'),text:`${label} · ${number(s.surviving_capacity_kw)} kW available / ${number(config.it_load_kw)} kW required`,healthy:s.service_maintained},...l].slice(0,50))}
    catch(e){setError(e instanceof Error?e.message:'Injection failed')}finally{setLabBusy(false)}
  }
  function scenario(s: string) {
    ...
    if(s==='path')ids=all.filter(c=>c.bank==='A'&&c.kind!=='CRAC').map(c=>c.id)
    if(s==='cooling')ids=all.filter(c=>c.kind==='CRAC').map(c=>c.id)
    void inject(ids,`Scenario: ${s==='path'?'power path A outage':s==='cooling'?'all cooling offline':`${s} UPS failure`}`)
  }
  ...
  function openHistory(r: Run) {setConfig(structuredClone(r.config));if(r.kind==='comparison')setComparison(r);else setRun(r);nav(r.kind==='comparison'?'comparison':'simulation');setToast('Saved experiment restored. Run again to reproduce it.')}
  ...
  const newExperiment=()=>{if(busy||labBusy)return;setConfig(structuredClone(DEFAULT_CONFIG));setRun(null);nav('simulation');setToast('New experiment ready. Configure your facility, then run the simulation.')}
  ...
  return <div className="app-shell">
    ...
    <main className="main-content"><div className="page-heading"><div><div className="heading-eyebrow"><i/> RESILIENCE ENGINEERING PLATFORM</div><h1>{TITLES[page][0]}</h1><p>{TITLES[page][1]}</p></div><div className="heading-actions">{['simulation','comparison'].includes(page)&&<div className="export-container"><button className="button" disabled={!activeRun||busy} onClick={()=>setShowExport(!showExport)} aria-expanded={showExport}><Download size={15}/>Export report<ChevronDown size={13}/></button>{showExport&&<><div className="menu-catcher" onClick={()=>setShowExport(false)}/><div className="export-menu"><button onClick={()=>exportActive('json')}><FileJson size={16}/><div>Complete experiment<small>JSON · parameters, sources & all trials</small></div></button><button onClick={()=>exportActive('csv')}><FileSpreadsheet size={16}/><div>Results summary<small>CSV · comparison-ready results</small></div></button><button onClick={()=>exportActive('trials')}><Database size={16}/><div>Individual trial data<small>CSV · all simulated trials</small></div></button><button onClick={()=>activeRun&&report(activeRun)}><Printer size={16}/><div>Printable design report<small>Print or save as PDF</small></div></button></div></>}</div>}<button className="button dark" disabled={busy||labBusy} onClick={newExperiment}><Plus size={16}/>New experiment</button></div></div>
      <div className="context-bar"><div><span><FlaskConical size={14}/>Monte Carlo simulation</span><i/><span>Independent failure model</span><i/><button onClick={()=>nav('references')}>IEEE 493–citing sources <ArrowUpRight size={12}/></button></div><span className="context-right"><ShieldCheck size={13}/>Open source. Reproducible.</span></div>
      ...
      {['simulation','comparison','lab'].includes(page)&&<div className={`workspace-grid ${page==='lab'?'lab-grid':''}`}><ConfigPanel config={config} setConfig={setConfig} busy={busy||labBusy} compare={page==='comparison'} onRun={()=>void execute(page==='comparison')}/><div className="workspace-main">
        {page==='simulation'&&<><FacilityDiagram state={topologyBusy?null:state} config={config} compact/><button className="lab-link" onClick={()=>nav('lab')}><Zap size={14}/><span>What happens if a component fails?</span><strong>Open live failure lab<ArrowUpRight size={14}/></strong></button></>}
        ...
        {busy&&page!=='lab'?<div className="running-panel panel" role="status" aria-live="polite"><div className="running-orbit"><FlaskConical size={31}/><i/><i/></div><span className="eyebrow">EXPERIMENT IN PROGRESS</span><h2>Putting your design to the test.</h2><p>Sampling failures, tracking repairs, and measuring service impact.</p><div className="indeterminate-progress"><i/></div><div className="running-meta"><span><LoaderCircle size={13} className="spin"/>NumPy simulation engine</span><span>{elapsed}s elapsed</span></div><small>Real calculations. No pre-computed reliability scores.</small></div>:null}
        {page==='simulation'&&!busy&&(run?<ResultsDashboard run={run} onCompare={()=>nav('comparison')}/>:<EmptyState icon={FlaskConical} title="Your next insight starts here." description="Configure a facility, choose your redundancy, and run thousands of independent failure trials." action="Run first simulation" onAction={()=>void execute()}/>)}
        {page==='comparison'&&!busy&&(comparison?<Comparison run={comparison}/>:<div className="comparison-empty"><div className="comparison-illustration"><span>N</span><span>N+1</span><span>2N</span><div className="comparison-baseline"/></div><span className="eyebrow">ONE EXPERIMENT. THREE ARCHITECTURES.</span><h2>Find your resilience sweet spot.</h2><p>Hold your facility load constant. Compare availability, downtime, SLA performance and infrastructure overhead across N, N+1 and 2N.</p><button className="button primary" onClick={()=>void execute(true)}><GitCompareArrows size={16}/>Run comparative study</button><div className="comparison-empty-notes"><span><CheckCircle2 size={14}/>Identical IT load</span><span><CheckCircle2 size={14}/>Shared random streams</span><span><CheckCircle2 size={14}/>Export-ready results</span></div></div>)}
        {page==='lab'&&<FailureLab config={config} state={topologyBusy?null:state} failed={failed} busy={labBusy||topologyBusy} onToggle={id=>void inject(failed.includes(id)?failed.filter(x=>x!==id):[...failed,id],`${failed.includes(id)?'Repaired':'Failed'} ${id}`)} onReset={()=>void inject([],'All components restored')} onScenario={scenario} logs={logs}/>}
      </div></div>}
...
function DesignReport({run}:{run:Run}) {return <article className="design-report"><span className="eyebrow">DC-RESILIENCE / EXPERIMENT REPORT</span><h1 id="report-title">{run.config.facility_name}</h1><p>Quantitative evaluation of data centre redundancy architecture</p><div className="report-meta"><span>Run: {run.run_id}</span><span>{new Date(run.created_at).toLocaleString()}</span></div><div className="data-warning"><Info size={17}/><p>Research demonstration with mixed-source data. PDU and cooling rates are illustrative; results are not publication-ready or a Tier certification.</p></div><h2>01 / Experiment configuration</h2><div className="report-config"><div>IT load<strong>{number(run.config.it_load_kw)} kW</strong></div><div>SLA benchmark<strong>Tier {run.config.tier_target}</strong></div><div>Trials per architecture<strong>{number(run.config.simulation.num_trials)}</strong></div><div>Years per trial<strong>{run.config.simulation.simulated_years_per_trial}</strong></div><div>Seed<strong>{run.config.simulation.seed}</strong></div><div>Scenario<strong>{run.config.simulation.operating_mode} · {run.config.simulation.stress_multiplier}×</strong></div><div>Failure mode<strong>{run.config.simulation.failure_mode}</strong></div><div>Engine version<strong>{run.engine_version}</strong></div></div><h2>02 / Results</h2><div className="table-scroll"><table><thead><tr><th>Architecture</th><th>Availability</th><th>Downtime / year</th><th>SLA breaches</th><th>Cost index</th></tr></thead><tbody>{run.results.map(r=><tr key={r.redundancy}><td><b>{r.redundancy}</b></td><td>{availability(r.availability_percent)}%</td><td>{number(r.expected_annual_downtime_minutes,3)} min</td><td>{number(r.sla_breach_rate_percent,3)}%</td><td>{number(r.infra_cost_index,2)}×</td></tr>)}</tbody></table></div>{run.results.map(r=><p className="report-ci" key={r.redundancy}>{r.redundancy}: approximate 95% mean availability CI [{r.availability_ci95.map(v=>v.toFixed(6)).join(', ')}]%. Exact trial-outage probability CI [{r.outage_probability_ci95.map(v=>(v*100).toFixed(4)).join(', ')}]%. {number(r.outage_trials)} outage trials, {number(r.overlap_trials)} overlap trials; expected unserved energy {number(r.expected_unserved_energy_kwh_per_year,2)} kWh/year.</p>)}<h2>03 / Failure-rate provenance</h2>{run.failure_rates.map(r=><div className="report-reference" key={r.component}><strong>{r.label} · λ = {r.failure_rate_per_year}/year · MTTR = {r.mttr_hours} h</strong><p>{r.source}</p><p>{r.notes} {r.source_url||''}</p></div>)}<h2>04 / Reproducibility & limitations</h2><ul>{run.model_notes.map(n=><li key={n}>{n}</li>)}</ul><p>Dataset: {run.dataset_version}. Detailed per-unit ratings, topology, all trial results and full provenance are included in the companion JSON export. This report is a summary, not a substitute for that reproducibility record.</p><div className="report-signoff"><strong>Romit Deokar & Shourya Saran</strong><span>SRM Institute of Science and Technology · 21CSE745P</span></div></article>}
